...
const fetchHistoricalData = async (coinId: string, days = 7) => {
  ...
  try {
    ...
    handleApiError(err, 'fetchHistoricalData');
  }
};
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="hidden lg:flex items-center space-x-4 text-sm" v-if="globalStats.total_market_cap?.usd">
            ...
            <div class="text-center">
              ...
              <p class="font-semibold">{{ globalStats.market_cap_percentage?.btc?.toFixed(1) || '0' }}%</p>
            </div>
          </div>
            ...
            <table class="w-full">
              ...
              <tbody>
                <tr v-for="coin in portfolioData" :key="coin.symbol" class="border-t border-gray-100">
                  ...
                  <td class="py-4">
                    {{ totalPortfolioValue > 0 ? ((coin.balance * coin.price / totalPortfolioValue) * 100).toFixed(1) :
                    '0' }}%
                  </td>
                </tr>
              </tbody>
        ...
        <div class="bg-white rounded-2xl p-6" v-if="globalStats.total_market_cap?.usd">
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-6">
            ...
            <div class="text-center">
              ...
              <p class="text-xl font-bold text-gray-800">${{ formatLargeNumber(globalStats.total_volume?.usd || 0) }}
              </p>
              <p class="text-sm text-blue-500">Trading</p>
            </div>
            <div class="text-center">
              <h4 class="text-sm text-gray-500 mb-2">Dominancia BTC</h4>
              <p class="text-xl font-bold text-gray-800">{{ globalStats.market_cap_percentage?.btc?.toFixed(1) || '0'
                }}%</p>
              <p class="text-sm text-orange-500">Bitcoin</p>
            </div>
            <div class="text-center">
              <h4 class="text-sm text-gray-500 mb-2">Dominancia ETH</h4>
              <p class="text-xl font-bold text-gray-800">{{ globalStats.market_cap_percentage?.eth?.toFixed(1) || '0'
                }}%</p>
              <p class="text-sm text-purple-500">Ethereum</p>
            </div>
            <div class="text-center">
              <h4 class="text-sm text-gray-500 mb-2">Cryptos Activas</h4>
              <p class="text-xl font-bold text-gray-800">{{ globalStats.active_cryptocurrencies?.toLocaleString() || '0'
                }}</p>
                  ...
                  <td class="py-4">
                    <div class="flex items-center space-x-2">
                      ...
                      <button @click="createPriceAlert(coin, coin.current_price * 1.1, 'above')"
                        class="text-blue-500 hover:text-blue-700 p-1 transition-colors" title="Crear Alerta">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                          <path
                            d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z">
                          </path>
      ...
      <div v-if="activeTab === 'alerts'" class="space-y-6">
        <div class="bg-white rounded-2xl p-6">
          ...
          <div v-if="priceAlerts.length === 0" class="text-center py-12 text-gray-500">
            ...
            <p class="text-sm">Crea tu primera alerta de precio para recibir notificaciones cuando tus monedas favoritas
              ...
              <div class="flex items-center">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500">
                    Alertar cuando el precio {{ alert.condition === 'above' ? 'suba por encima de' : 'baje por debajode'
